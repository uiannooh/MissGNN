...
def get_gnn(data, args, params):
    model_types = params["model_types"].split('_')
    if args.norm_embs is None:
        norm_embs = [True,]*len(model_types)
    else:
        norm_embs = list(map(bool,map(int,args.norm_embs.split('_'))))
    if params["post_hiddens"] is None:
        post_hiddens = [params["node_dim"]]
    else:
        post_hiddens = list(map(int, params["post_hiddens"].split('_')))
    # print(model_types, norm_embs, post_hiddens)
    # build model
    model = GNNStack(data.num_node_features, data.edge_attr_dim,
                        params["node_dim"], params["edge_dim"], args.edge_mode,
